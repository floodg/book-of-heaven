...
/** Single-line excerpt for chat markdown; centers on query terms when found. */
export function excerptForSearchHit(
  ...
  const innerBudget = Math.max(80, maxLen - 2) // room for … prefix/suffix
...
export function citationForHit(hit: SearchHit): string {
  ...
  const display = typeof meta.entry_date_display === 'string' ? meta.entry_date_display.trim() : ''
  if (display) {
    return `[VOLUME${padVolume2(hit.volume)}.pdf - ${display}]`
  }
  if (hit.entry_date) {
    return `[VOLUME${padVolume2(hit.volume)}.pdf - ${String(hit.entry_date)}]`
  }
...
export function hitsToReplyMarkdown(
  ...
  hits.forEach((hit, i) => {
    ...
    const sim = typeof hit.similarity === 'number' ? hit.similarity.toFixed(3) : '?'
    const clipped = excerptForSearchHit(hit.chunk_text, queryHint ?? undefined, MARKDOWN_HIT_EXCERPT_MAX)
    lines.push(`${i + 1}. ${cite} _(similarity ${sim})_`)
    lines.push('')
    lines.push(`> ${clipped}`)
    lines.push('')
  })
